    ...
    context('Footer and list of stories', () => {
      ...
      context('List of stories', () => {
        ...
        it('shows the right data for all rendered stories', () => {
          cy.get('.item').first().should('be.visible').and('contain', stories.hits[0].title).and('contain', stories.hits[0].author)
          cy.get('.item').last().should('be.visible').and('contain', stories.hits[1].title).and('contain', stories.hits[1].author)
          cy.get(`.item a:contains(${stories.hits[0].title})`).should('be.visible').and('have.attr', 'href', stories.hits[0].url)
          cy.get(`.item a:contains(${stories.hits[1].title})`).should('be.visible').and('have.attr', 'href', stories.hits[1].url)
        })

        it('shows only one less stories after dimissing the first story', () => {
        ...
        context('Order by', () => {
          it('orders by title', () => {
            ...
            cy.get(`.item a:contains(${stories.hits[0].title})`).should('have.attr', 'href', stories.hits[1].url)

          })

          it('orders by author', () => {
            cy.get('.list-header-button:contains(Author)').as('titleAuthor').click()
            cy.get('.item').first().should('be.visible').and('contain', stories.hits[0].author)

            cy.get('@titleAuthor').click()
            cy.get('.item').first().should('be.visible').and('contain', stories.hits[1].author)
          })

          it('orders by comments', () => {
            cy.get('.list-header-button:contains(Comments)').as('titleComments').click()
          ...
          it('orders by points', () => {
            cy.get('.list-header-button:contains(Points)').as('titlePoints').click()
...
it('shows a "Loading ..." state before showing the results', () => {
  cy.intercept('GET', '**/search**', { delay: 1000, fixture: 'stories'}).as('getDelayStories')
  cy.visit('/')
